...
After Int4 quantization in Appendix~\ref{app:quantization}, MiniCPM-2.4B's footprint is reduced to 2GB, facilitating deployment on mobile edge devices. We adapted the model for Android and HarmonyOS using \uline{MLC-LLM}~\citep{mlc-llm} and employed \uline{LLMFarm}~\citep{llmfarm} for adaptation to the iPhone system. This adaptation was tested on various edge mobile devices.

The result is shown in Table~\ref{table:phone_specs}, we can see that on the most advanced smartphone iPhone 15 Pro, \uline{the inference throughput is as high as 18 tokens per second.} In other devices, the inference throughput is also acceptable.
